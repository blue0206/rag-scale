...
def process_chunks(data: EmbeddingJob) -> None:
    ...
    try:
        ...
        asyncio.run(update_embedding_status(data.user_id, data.batch_id, len(data.payload)))
...
async def update_embedding_status(user_id: str, batch_id: str, n: int) -> None:
    ...
    await batch_tracking_service.increment_field(batch_id=batch_id, field="chunks_embedded", delta=n)
    ...
    if batch_status.chunks_embedded == batch_status.total_chunks and batch_status.files_chunked == batch_status.total_files:
    ...
    # If not all chunks are embedded, publish progress update.
        ...
        progress = int((batch_status.chunks_embedded / batch_status.total_chunks) * 100) if batch_status.total_chunks > 0 else 0
